...
def calculate_two_arrow_bezier_with_new_point(P0, P1, P2, P3, P4, P5, number_of_steps):
    ...
    # New segment P3 -> P4 -> P5
    control_x2, control_y2 = calculate_control_point(P3[0], P3[1], P4[0], P4[1], 1)  # Adjust distance
    x2, y2 = calculate_bezier(P3, (control_x2, control_y2), P4, number_of_steps)

    # Ensure the Bezier curve from P4 to P5 is a straight line
    x3, y3 = zip(*[(P4[0] + (P5[0] - P4[0]) * t, P4[1] + (P5[1] - P4[1]) * t) for t in
                   [i / number_of_steps for i in range(number_of_steps + 1)]])

    return x0, y0, x1, y1, x2, y2, x3, y3, control_x2, control_y2


def draw_curved_and_inverted(x0, y0, x1, y1, x2, y2, x3, y3, inv_x2, inv_y2, P0, P1, P2, P3, P4, P5, width=800,
                             height=600, x_scale=100, y_scale=100, x_offset=100, y_offset=100, radius=10):
    ...
    pydraw.Oval(screen, (x_offset + x_scale * P4[0]) - radius / 2, height - (y_offset + y_scale * P4[1] + radius / 2),
                radius, radius)
    pydraw.Oval(screen, (x_offset + x_scale * P5[0]) - radius / 2, height - (y_offset + y_scale * P5[1] + radius / 2),
                radius, radius)
...
def main():
    ...
    x0, y0, x1, y1, x2, y2, x3, y3, inv_x2, inv_y2 = calculate_two_arrow_bezier_with_new_point(P0, P1, P2, P3, P4, P5,
                                                                                               number_of_steps)
    draw_curved_and_inverted(x0, y0, x1, y1, x2, y2, x3, y3, inv_x2, inv_y2, P0, P1, P2, P3, P4, P5)
