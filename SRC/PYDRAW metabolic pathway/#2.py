...
def calculate_bezier_Q_t(P0, P1, P2, t):
    # Perform the Bezier calculation for each dimension (x, y)
...
def calculate_bezier(P0, P1, P2, number_of_steps):
    ...
    x = [coord[0] for coord in bezier_coords]
    y = [coord[1] for coord in bezier_coords]
    return x, y


# Draw function that utilizes pydraw
def draw(screen, x0, y0, x1, y1, nodes, edges, width=800, height=600, x_scale=50, y_scale=50, x_offset=100,
         y_offset=100):
    ...
    for edge in edges:
        pydraw.Line(screen, x_scale * float(nodes[edge[0]][0]) + x_offset,
                    height - y_offset - y_scale * float(nodes[edge[0]][1]),
                    x_offset + x_scale * float(nodes[edge[1]][0]),
                    height - y_offset - y_scale * float(nodes[edge[1]][1]))
