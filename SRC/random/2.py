...
def calculate_two_arrow_bezier_with_new_point(P0, P1, P2, P3, P4, number_of_steps):
    ...
    # Straight line segment from P4 to P5
    x2 = [P4[0] + (P4[0] - P3[0]) * t for t in [i / number_of_steps for i in range(number_of_steps + 1)]]
    y2 = [P4[1] + (P4[1] - P3[1]) * t for t in [i / number_of_steps for i in range(number_of_steps + 1)]]

    return x0, y0, x1, y1, x2, y2


def draw_curved(x0, y0, x1, y1, x2, y2, P0, P1, P2, P3, P4, width=800, height=600, x_scale=100, y_scale=100,
                x_offset=100, y_offset=100, radius=10, grid_size=50):
    screen = pydraw.Screen(width, height)

    # Draw grid lines
    for x in range(0, width, grid_size):
        pydraw.Line(screen, x, 0, x, height, color=(200, 200, 200))
    for y in range(0, height, grid_size):
        pydraw.Line(screen, 0, y, width, y, color=(200, 200, 200))
...
def main():
    ...
    screen = draw_curved(x0, y0, x1, y1, x2, y2, P0, P1, P2, P3, P4)
